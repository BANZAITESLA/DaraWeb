...
<body>
    ...
    <?php
        ...
        if($db->connect_errno==0) {
            ...
            $res = $db->query("SELECT * FROM pegawai JOIN jabatan ON pegawai.id_jabatan = jabatan.id_jabatan JOIN status_pegawai ON status_pegawai.id_status_p = pegawai.id_status_p WHERE id_pegawai = '$id';");
            if($res){
                if($res->num_rows>0){
...
?>
    ...
    <script>
        $('#formData').on('submit', function(e) {
            ...
            let dest = "data-diri-pegawai.php";
            $.ajax({
                method: "POST",
                url: url,
                data: {
